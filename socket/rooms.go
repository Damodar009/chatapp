	...
	for {
		...
		case client := <-r.Register:
			...
			for client := range r.Client {
				if err := client.Conn.WriteJSON(Message{Type: "added", Message: "new user added"}); err != nil {
				}
			}
		case client := <-r.Unregister:
			if _, ok := r.Client[client]; ok {
				...
				for client := range r.Client {
					...
					if err := client.Conn.WriteJSON(Message{Type: "removed", Message: "user removed"}); err != nil {
					}
				}
			}
